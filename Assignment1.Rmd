...
Several noteworthy takeaways emerge from the analysis. Firstly, education emerges as a robust predictor of home value, a finding of particular interest in a city characterized by a dominant "meds and eds" industry. Secondly, while high vacancy rates are common in this post-industrial city, the data suggests that vacancy may not exert a strong influence on home values, possibly due to its widespread prevalence. Similarly, the presence of single homes does not appear to significantly impact home values, a trend that may be linked to gentrification dynamics where the immediate neighborhood composition may matter less. Finally, the strong predictive power of household income and poverty rate in a highly racially and socioeconomically segregated city aligns with expectations, as these variables are closely tied to the overall living environment and quality, shedding light on the intricate interplay between socio-demographic factors and housing values in this urban setting.

Limitations:
- high spatial autocorrelation in errors, meaning the generalizability of this model is low and it doesn't predict well across different types of neighborhood contexts
- limited sample size inherent in census data - requires people to response
- skewed sample (low-income, low-ses, minority, non english speaking communities are less likely to answer census, leading to lower representation in neighborhoods)
- heterscedasticity in standard error - 
- unclear what the remaining 33% of 
...
Model Evaluation:
Based on the results of our regression, the 4 predictors are all considered as significant in predicting the median home value. Therefore, the model is relatively robust and each predictor can explain some of the variance of the dependent variable.
The k-fold cross-validation result tells that model with 4 predictors has a lower root mean square error (RMSE) than model with only 2 predictors, which suggests that more predictors help to improve the predicting ability of variance of the dependent variable of median home value.
The other predictor that can be associate with our model can be safety. Safer neighborhoods are generally considered to have a higher median home value than areas with more crimes.

Assumptions Violations:
The scatterplots of the dependent variable and predictors indicate violations of the linearity and homoscedasticity assumptions. Additionally, the histogram of standardized residuals suggests a slight left skew, which may affect the accuracy of the model's predictions. Moreover, the multicollinearity in the correlation matrix implies inaccuracy of the model. Also, utilizing the raw number of households living in poverty (NBELPOV100) as a predictor might skew the impact of poverty on median home values for it does not account for the variance of population size across regions. 

Ridge or LASSO regression?
The Ridge or LASSO regression are appropriate here. They can help addressing the multicollinearity issue in the correlation matrix. Ridge regression and LASSO regression are capable of shrink penalty on the regression coefficient estimates. Shrinkage penalty term has no effect, and regressions will produce the least squares estimates, so it could help stabilize the coefficients and improve the model's interpretability.
