...
Our regression analysis found that the aforementioned variables explain approximately 66% of the variance in log-transformed median home values within Philadelphia. Notably, educational attainment (PCTBACHMOR) emerges as the strongest predictor. A 1% increase in PCTVACANT and LNNBELPOV100 respectively lead to a \$191.57 and \$789.05 decrease in median home value, while a 1% increase in PCTSINGLES and PCTBACHMOR respectively leads to a \$29.77 and \$209.10 increase in median home value. 

A critical observation lies in the spatial heteroscedasticity, signifying high clustering in errors. This hints at our model's lower predictive power in certain local contexts. Considering these limitations, further research and construction of a holistic, city-wide model is needed to capture the intricate dynamics influencing housing values across Philadelphia's diverse neighborhoods.
...
In our analytic framework, we use multiple regression to investigate the relationship between our dependent variable, median home value, and various explanatory variables. Through this approach, we not only ascertain the magnitude and direction of the relationship between each predictor and the median home value, but also assess the overall fit of the model to our data. Crucially, this methodology permits us to examine individual predictors in isolation and explore how median home value may change in association with a unitary change in a given predictor.
...
$$ \text{LNMEDHVAL}_i = \beta_0 + \beta_1(\text{PCTBACHMOR}_i) + \beta_2(\text{LNNBELPOV100}_i) + \beta_3(\text{PCTVACANT}_i) + \beta_4(\text{PCTSINGLES}_i) + \epsilon_i $$

Where $\beta_0$ represents the value of the log-transformed median home value when all of the predictors equal 0. The beta coefficient preceding each independent variable denotes the direction and amount by which log-transformed median home value changes when the predictor increases by one unit. \( \epsilon_i \) represents the error term for the \( i^{th} \) observation.
...
The values of $\beta _{0}$ $\beta _{i}$ are chosen simultaneously to minimize the expression for the Error Sum of Squares (SSE), which is the squared distance between $\beta_{k}$ and the regression line, or the residual. Their values are calculated as follows, and we receive least squares values of the beta coefficients when the SSE is minimized. 
...
The regression consists of the natural logarithm of median home value (LNMEDHVAL) regressed on:
- percent of vacant homes (PCTVACANT)  
...
The estimate of PCTVACANT is -0.0191569, which demonstrates that a 1-unit increase (i.e., percentage point)  in the percentage of vacant homes is associated with a decrease of $\beta _{3}$ = 0.0191569 in the response variable of log-transformed median home value. Similarly, a 1-unit increase (i.e., percentage point) in the log-transformed share of households in poverty (LNNBELPOV100) is associated with a decrease of $\beta _{2}$ = 0.0789054 in the median home value. In practical terms, a 1% increase in PCTVACANT and LNNBELPOV100 respectively lead to a $\beta _{3}$ = \$191.57 and $\beta _{2}$ = \$789.05 decrease in median home value.
...
PCTSINGLES and PCTBACHMOR have the estimates of 0.0029769 and 0.209098 respectively, which implies that a 1-unit increase(i.e., percentage point) in the percentage of single home units and the percentage of individuals with a bachelor's degree is individually associated with an increase of $\beta _{4}$ = 0.0029769 and $\beta _{1}$ =  0.209098 in log-transformed median home value. In other words, a 1% increase in PCTSINGLES and PCTBACHMOR respectively leads to a $\beta _{4}$ = \$29.77 and $\beta _{1}$ = \$209.10 increase in median home value.


The log-transformed regression model has a multiple $R^2$ of 0.6623 and adjusted $R^2$ of 0.6615, which means that approximately 66.23% of the log-transformed values of median home value can be explained by the predictors in the model. This $R^2$ indicates a moderately strong relationship between median home value and the model predictors. All of our four independent variables are also statistically significant, with p-values well below 0.001. This tells us that for all of our independent variables, the odds of them being equal to their respective beta coefficient estimates in the case that the null hypothesis $\H _{0}$ is true is less than 0.001. Given these low probabilities, we can reject the null hypothesis $\H _{0}$ that all of our beta coefficients are equal to zero in favor of the alternative hypothesis $\H _{a}$ that at least one of our beta coefficients is not equal to zero. The low p-value associated with the F-statistic tells us that we can reject the null hypothesis $\H _{0}$ that all of our beta coefficients are equal to zero.
...
The following histogram shows the frequency distribution of standardized residuals. A standardized residual represents the original residual value divided by the standard deviation of all residuals, producing a standardized value that represents a given residual's distance from the line of best fit. Based on the shape of this distribution, we can estimate that residuals have a slight skew to the left and is not normal, indicating that the median is greater than the mean.
...
Based on the previous maps of LNMEDHVAL and predictor variables across Philadelphia, there seems to be a great deal of spatial autocorrelation in our data. All variables show spatial clustering to some extent, suggesting that block groups are not independent of each other with respect to these variable values and are likely spatially auto-correlated.
...
Our final model yielded an adjusted \( R^2 \) of 0.66. This suggests that our selected predictors account for 66% of the variance in median home values at the census block group level in Philadelphia. All p-values associated with our beta coefficient estimates were less than 0.001, allowing us to reject the null hypothesis that all of our beta coefficients equal zero. This is not surprising, as all of the predictors are commonly known to be associated with home prices and are commonly discussed in academia and real estate. 
...
- **Variable selection**: The model is also limited by our variable selection, specifically because we use the raw count of households living in poverty rather than the percentage. As a result, this variable may have an outsized effect in block groups with more households, since it's not normalized by population. 
